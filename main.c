...
GtkDrawingArea *da;
GtkToggleButton *rb_Pace;
GtkToggleButton *rb_Cadence;
GtkToggleButton *rb_HeartRate;
GtkToggleButton *rb_Altitude;
GtkToggleButton *rb_Splits;
GtkButton *btnFileOpen;
GtkFrame *viewport;
 ...
 */
void
raw_to_user_plots (PlotData *pdest, int num_recs, float x_raw[NSIZE],
...
void
cadence_plot_labeler (PLINT axis, PLFLT value, char *label, PLINT length,
                      PLPointer label_data)
{
  if (axis == PL_Y_AXIS)
    snprintf (label, (size_t)length, "%3.2f", value);
...
void
heart_rate_plot_labeler (PLINT axis, PLFLT value, char *label, PLINT length,
                         PLPointer label_data)
{
  if (axis == PL_Y_AXIS)
...
void
altitude_plot_labeler (PLINT axis, PLFLT value, char *label, PLINT length,
                       PLPointer label_data)
{
  if (axis == PL_Y_AXIS)
    snprintf (label, (size_t)length, "%3.0f", value);
...
static void
on_da_right_btn_drag_update (GtkGestureDrag *gesture,
                                   double             dx,
                                   double             dy,
                                   AllData*           data)
{
...
//void
//findCenter (int numPts, double lat[], double lng[], double center[],
//            float *min_lat, float *min_lng, float *max_lat, float *max_lng)
//{
//  *min_lat = DBL_MAX;
//  *max_lat = -DBL_MAX;
//  *min_lng = DBL_MAX;
 ...
 */
//void
//mapLimits (float *min_map_lat, float *min_map_lng, float *max_map_lat,
//           float *max_map_lng)
//{
...
#endif
void
on_btnFileOpen_clicked (GtkButton *btnFileOpen, AllData *pall)
{
...
on_window_show_about_dialog (GtkButton *btn, GtkWindow *window)
{
  ...
  const gchar *authors[]
      = { "Craig S. Prevallet", "John Stowers", "Tormod Erevik Lea", NULL };
...
main (int argc, char *argv[])
{
  ...
  rb_Pace = GTK_TOGGLE_BUTTON (gtk_builder_get_object (builder, "rb_Pace"));
  rb_Cadence
      = GTK_TOGGLE_BUTTON (gtk_builder_get_object (builder, "rb_Cadence"));
  rb_HeartRate
      = GTK_TOGGLE_BUTTON (gtk_builder_get_object (builder, "rb_HeartRate"));
  rb_Altitude
  ...
  g_signal_connect (GTK_TOGGLE_BUTTON (rb_HeartRate), "toggled",
                    G_CALLBACK (on_rb_heartrate), pall);
  g_signal_connect (GTK_TOGGLE_BUTTON (rb_Altitude), "toggled",
                    G_CALLBACK (on_rb_altitude), pall);
  g_signal_connect (GTK_TOGGLE_BUTTON (rb_Splits), "toggled",
...
//                    NULL);
  ...
  g_signal_connect (GTK_BUTTON (btnFileOpen), "clicked",
                    G_CALLBACK (on_btnFileOpen_clicked), pall);
